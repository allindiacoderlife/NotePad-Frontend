...
interface IShape extends IPaintStyle {
  tool: string;
  coordinates: any;
  paint: any; // Add the paint property
}
...
const useCanvas = ({ paintStyle, tool }: { paintStyle: IPaintStyle; tool: string }) => {
  ...
  const save = () => {
    const canvasData = shapes.map((shape) => ({
      ...shape,
      coordinates: {
        ...shape.coordinates,
        path: shape.coordinates.path ? shape.coordinates.path.toSVGString() : undefined,
      },
    }));
    const image = innerRef.current?.toDataURL('image/png');
    const json = JSON.stringify(canvasData);
    console.log('Canvas saved:', json);
  }
